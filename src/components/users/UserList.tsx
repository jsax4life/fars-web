...
const UserList = () => {
  ...
  const [role, setRole] = useState<'admin' | 'super_admin' | 'staff'>('staff');
  const [users, setUsers] = useState<User[]>([
    {
      ...
      avatarUrl: "https://via.placeholder.com/30/0000FF/808080?Text=JD",
    },
    {
      id: "2",
      firstName: "Jane",
      ...
      isActive: false,
      createdAt: new Date().toISOString(),
      avatarUrl: "https://via.placeholder.com/30/FF0000/FFFFFF?Text=JS",
    },
    {
      id: "3",
      firstName: "Peter",
      ...
      isActive: true,
      createdAt: new Date().toISOString(),
      avatarUrl: "https://via.placeholder.com/30/008000/FFFFFF?Text=PJ",
    },
    {
      id: "4",
      firstName: "Alice",
      ...
      isActive: true,
      createdAt: new Date().toISOString(),
      avatarUrl: "https://via.placeholder.com/30/FFA500/000000?Text=AB",
    },
  ]);
        ...
        <div className="bg-white rounded-lg w-full shadow overflow-hidden">
          <div className="p-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex flex-col sm:flex-row w-full md:w-auto gap-2 sm:gap-4">
              <div className="relative w-full sm:w-auto">
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value as 'admin' | 'super_admin' | 'staff')}
          ...
          {showCreateModal && (
            <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-md overflow-y-auto max-h-[90vh]"> {/* Added overflow-y and max-h */}
          ...
          {showDeactivateConfirm && (
            <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md overflow-y-auto max-h-[90vh]"> {/* Added overflow-y and max-h */}
                <div className="mb-4">
          ...
          {showSuccessModal && (
            <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-md text-center overflow-y-auto max-h-[90vh]"> {/* Added overflow-y and max-h */}
          ...
          {showViewModal && viewedUser && (
            <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50 p-4">
              <div className="bg-gray-50 p-6 rounded-md shadow-md w-full max-w-2xl overflow-y-auto max-h-[90vh]"> {/* Added overflow-y and max-h */}
                ...
                <div className="mb-6 p-4 bg-white rounded-md border border-gray-200">
                  <div className="md:grid md:grid-cols-2 md:items-start md:gap-6 mb-4"> {/* Flex layout for label and text */}
                    ...
                    <p className="text-xs text-gray-500 md:mt-1">View the personal information of this user.</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="tel" id="phone" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={viewedUser.phone || 'N/A'} readOnly />
                    </div>
                    <div>
                      <label htmlFor="role" className="block text-xs font-medium text-gray-600 mb-1">Role</label>
                      <input type="text" id="role" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={viewedUser.role} readOnly />
                    </div>
                    <div>
                      <label htmlFor="status" className="block text-xs font-medium text-gray-600 mb-1">Status</label>
                      <input type="text" id="status" className={`w-full px-3 py-2 border border-gray-300 rounded-md text-sm font-semibold ${viewedUser.isActive ? 'text-green-600 border-green-300' : 'text-red-600 border-red-300'} focus:outline-none focus:border-orange-500`} value={viewedUser.isActive ? 'Active' : 'Inactive'} readOnly />
                    </div>
                    <div className="col-span-2">
                      <label htmlFor="createdat" className="block text-xs font-medium text-gray-600 mb-1">Date Added</label>
                      <input type="text" id="createdat" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={new Date(viewedUser.createdAt).toLocaleString()} readOnly />
                    </div>
                    {viewedUser.phone && (
                      <div className="col-span-2">
                        <label htmlFor="full-phone" className="block text-xs font-medium text-gray-600 mb-1">Full Phone Number</label>
                        <div className="flex items-center border border-gray-300 rounded-md">
                          <div className="px-3 py-2 bg-gray-100 text-sm text-gray-700 rounded-l-md">
                            {/* You might want to display a country code here if available */}
                            +234
                          </div>
                          <input type="tel" id="full-phone" className="w-full px-3 py-2 text-sm text-gray-700 focus:outline-none" value={viewedUser.phone} readOnly />
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex justify-end">
                  <button onClick={() => setShowViewModal(false)} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md text-sm font-medium text-gray-700 focus:outline-none">
                    Close
                  </button>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-6 rounded-md shadow-md w-full max-w-2xl overflow-y-auto max-h-[90vh]"> {/* Added overflow-y and max-h */}
                ...
                <div className="mb-6 p-4 bg-white rounded-md border border-gray-200">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="firstname" className="block text-xs font-medium text-gray-600 mb-1">Firstname</label>
                      <input type="text" id="firstname" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={editedUser.firstName} readOnly />
                    </div>
                    <div>
                      <label htmlFor="lastname" className="block text-xs font-medium text-gray-600 mb-1">Lastname</label>
                      <input type="text" id="lastname" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={editedUser.lastName} readOnly />
                    </div>
                    <div className="col-span-2">
                      <label htmlFor="email" className="block text-xs font-medium text-gray-600 mb-1">Email Address</label>
                      <input type="email" id="email" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={editedUser.email} readOnly />
                    </div>
                    <div>
                      <label htmlFor="phone" className="block text-xs font-medium text-gray-600 mb-1">Phone Number</label>
                      <input type="tel" id="phone" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={editedUser.phone || 'N/A'} readOnly />
                    </div>
                    <div>
                      <label htmlFor="role" className="block text-xs font-medium text-gray-600 mb-1">Role</label>
                      <input type="text" id="role" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={editedUser.role} readOnly />
                    </div>
                    <div>
                      <label htmlFor="status" className="block text-xs font-medium text-gray-600 mb-1">Status</label>
                      <input type="text" id="status" className={`w-full px-3 py-2 border border-gray-300 rounded-md text-sm font-semibold ${editedUser.isActive ? 'text-green-600 border-green-300' : 'text-red-600 border-red-300'} focus:outline-none focus:border-orange-500`} value={editedUser.isActive ? 'Active' : 'Inactive'} readOnly />
                    </div>
                    <div className="col-span-2">
                      <label htmlFor="createdat" className="block text-xs font-medium text-gray-600 mb-1">Date Added</label>
                      <input type="text" id="createdat" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 focus:outline-none focus:border-orange-500" value={new Date(editedUser.createdAt).toLocaleString()} readOnly />
                    </div>
                    {editedUser.phone && (
                      <div className="col-span-2">
                        <label htmlFor="full-phone" className="block text-xs font-medium text-gray-600 mb-1">Full Phone Number</label>
                        <div className="flex items-center border border-gray-300 rounded-md">
                          ...
                          <input type="tel" id="full-phone" className="w-full px-3 py-2 text-sm text-gray-700 focus:outline-none" value={editedUser.phone} readOnly />
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex justify-end">
                  <button onClick={() => setShowEditModal(false)} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md text-sm font-medium text-gray-700 focus:outline-none">
                    Close
                  </button>
                </div>
              </div>
